...
public class MatchingController : MonoBehaviour
{
    ...
    public static MatchingController Instance
    {
        get
        {
            if (_instance == null)
            {
                GameObject matchingController = new GameObject("MatchingController");
                matchingController.AddComponent<MatchingController>();
            }
            return _instance;
        }
    }
    ...
    [Space]
    [Header("Answers")]
    [SerializeField] private List<AnswerZoneMatch> answerZones; //To identify which answer has been chosen by the player
    ...
    private void ShuffleAnswerAndDisplayQuestion()
    {
        ...
        List<AnswerZoneMatch> tempList = new List<AnswerZoneMatch>(answerZones);
        ...
        while (tempList.Count > 0)
        {
            for (int i = Random.Range(0,tempList.Count); i < tempList.Count; i++)  //Random where to put the words
            {
                ...
                {
                    DisplayAnswerText(tempList[i].answerText, tempQuestionList[currentQuestion].correctWord);
                ...
                else
                {
                    ...
                    DisplayAnswerText(tempList[i].answerText,tempQuestionList[newRandom].correctWord);
